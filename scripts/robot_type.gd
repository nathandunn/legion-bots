...
## somewhere else - the same budget idea as the builds in Dodgeball Bots, and the same five
## properties as its armed cousin Melee Bots, so a type means the same thing in both.
##
...
const PROP_HELP := {
	"brawn": "Damage: punches, kicks, and how hard a stone leaves the hand",
	...
	"aim": "Throwing accuracy and how well it leads a running target",
}
...
const TYPE_HELP := {
	...
	"Bruiser": "Hits like a truck and can take one; slow, and a terrible shot",
	"Runner":  "First to every loose stone, glass jaw",
	"Tank":    "Soaks punishment and keeps coming; ponderous",
	"Sniper":  "Lands stones from across the arena; folds up close",
